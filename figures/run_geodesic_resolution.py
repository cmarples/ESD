...
for i in range(len(no_vertices)):
    print("Resolution no.", i+1, "out of 25")
    for j in range(len(shape)):
        
        ### Polar grid with 4-neighbours
        # Generate 4-neighbour polar grid.
        tic = time.perf_counter()
        grid = leod.fmm.grid_pol.gen_pol_grid(no_theta[i], no_phi[i], shape[j], is_connect_8=False)
        # Precalculation on 4-neighbour polar grid.
        toc = time.perf_counter()
        t_polar_gen_4[i][j] = toc - tic
        # Dijkstra's algorithm on the 4-neighbour grid.
        tic = time.perf_counter()
        d_dijkstra_4[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=True)
        toc = time.perf_counter()
        t_dijkstra_4[i][j] = toc - tic
        # Fast marching on the 4-neighbour grid.
        tic = time.perf_counter()
        d_fmm_4[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=False)
        toc = time.perf_counter()
        t_fmm_4[i][j] = toc - tic
        ...
        tic = time.perf_counter()
        grid = leod.fmm.grid_pol.gen_pol_grid(no_theta[i], no_phi[i],  shape[j], is_connect_8=True)
        toc = time.perf_counter()
        t_polar_gen_8[i][j] = toc - tic
        # Dijkstra's algorithm on the 8-neighbour grid.
        tic = time.perf_counter()
        d_dijkstra_8[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=True)
        toc = time.perf_counter()
        t_dijkstra_8[i][j] = toc - tic
        # Fast marching on the 8-neighbour grid.
        tic = time.perf_counter()
        d_fmm_8[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=False)
        toc = time.perf_counter()
        t_fmm_8[i][j] = toc - tic
        
        ### Icosahedral triangulation
        tic = time.perf_counter()
        grid = leod.fmm.grid_ico.gen_ico_grid(no_divisions[i], shape[j], is_generic=False)
        toc = time.perf_counter()
        t_tri_gen[i][j] = toc - tic
        
        # Dijkstra's algorithm on triangulation.
        tic = time.perf_counter()
        d_dijkstra_tri[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=True)
        toc = time.perf_counter()
        t_dijkstra_tri[i][j] = toc - tic
        # Fast marching on triangulation.
        tic = time.perf_counter()
        d_fmm_tri[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=False)
        toc = time.perf_counter()
        t_fmm_tri[i][j] = toc - tic
        
        ### Icosahedral triangulation with splitting
        # Genertae grid and split obtuse angles.
        tic = time.perf_counter()
        grid = leod.fmm.grid_ico.gen_ico_grid(no_divisions[i], shape[j], is_generic=False)
        leod.fmm.grid_pre.split_obtuse_angles(grid.vertex)
        toc = time.perf_counter()
        t_tri_split_gen[i][j] = toc - tic
        
        # Dijkstra's algorithm on triangulation.
        tic = time.perf_counter()
        d_dijkstra_split[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=True)
        toc = time.perf_counter()
        t_dijkstra_split[i][j] = toc - tic
        # Fast marching on triangulation.
        tic = time.perf_counter()
        d_fmm_split[i][j], fmm = leod.fmm.callers.distance_pair(shape[j], grid, start_point, end_point, is_dijkstra=False)
        toc = time.perf_counter()
...
for j in range(len(shape)):
    file_name = 'data/geodesics/resolution/resolution_' + shape_name[j] + '.txt'
    with open(file_name, mode="w", newline='') as f:
        ...
        for i in range(len(no_vertices)):
            row = [ d_dijkstra_4[i][j], d_fmm_4[i][j], d_dijkstra_8[i][j], d_fmm_8[i][j],
                    ...
                    t_tri_gen[i][j], t_dijkstra_tri[i][j], t_fmm_tri[i][j],
