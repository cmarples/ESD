...
class TestBVM(unittest.TestCase):
    ...
    def test_boundary_value_method_table_1(self):
        ...
        dp = 3 # Number of decimal places used in comparison
        
        s = bvm_dist(E, [0.0, 0.0], [0.0, 90.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 10018754.9569, dp, "Expect d = 10018754.9569 m")
        
        s = bvm_dist(E, [1.0*conv, 0.0], [-80.0*conv, 5.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 8947130.7221, dp, "Expect d = 8947130.7221 m")
        
        s = bvm_dist(E, [5.0*conv, 0.0], [-60.0*conv, 40.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 8004762.4330, dp, "Expect d = 8004762.4330 m")
        
        s = bvm_dist(E, [30.0*conv, 0.0], [-30.0*conv, 175.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 19547128.7971, dp, "Expect d = 19547128.7971 m")
        
        s = bvm_dist(E, [60.0*conv, 0.0], [60.0*conv, 175.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 6705715.1610, dp, "Expect d = 6705715.1610 m")
        
        s = bvm_dist(E, [75.0*conv, 0.0], [80.0*conv, 120.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 2482501.2608, dp, "Expect d = 2482501.2608 m")
        
        s = bvm_dist(E, [80.0*conv, 0.0], [60.0*conv, 90.0*conv], is_radians=True, is_jacobi=True, n=16000)      
    ...
    def test_boundary_value_method_table_2(self):
        ...
        s = bvm_dist(E, [0.0, -90.0*conv], [0.0, 89.5*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 19981849.8629, dp, "Expect d = 19981849.8629 m")
        
        s = bvm_dist(E, [1.0*conv, -90.0*conv], [1.0*conv, 89.5*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 19776667.0342, dp, "Expect d = 19776667.0342 m")
        
        s = bvm_dist(E, [5.0*conv, -90.0*conv], [5.0*conv, 89.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 18889165.0873, dp, "Expect d = 18889165.0873 m")
        
        s = bvm_dist(E, [30.0*conv, -90.0*conv], [30.0*conv, 86.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 13331814.6078, dp, "Expect d = 13331814.6078 m")
        
        s = bvm_dist(E, [60.0*conv, -90.0*conv], [60.0*conv, 78.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 6637321.6350, dp, "Expect d = 6637321.6350 m")
        
        s = bvm_dist(E, [75.0*conv, -90.0*conv], [75.0*conv, 66.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 3267941.2812, dp, "Expect d = 3267941.2812 m")
        
        s = bvm_dist(E, [80.0*conv, -90.0*conv], [80.0*conv, 55.0*conv], is_radians=True, is_jacobi=True, n=16000)      
    ...
    def test_boundary_value_method_table_3(self):
        ...
        s = bvm_dist(E, [0.0, 0.5*conv], [80.0*conv, 0.5*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 8831874.3717, dp, "Expect d = 8831874.3717 m")
        
        s = bvm_dist(E, [-1.0*conv, 5.0*conv], [75.0*conv, 5.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 8405370.4947, dp, "Expect d = 8405370.4947 m")
        
        s = bvm_dist(E, [-5.0*conv, 30.0*conv], [60.0*conv, 30.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 7204083.8568, dp, "Expect d = 7204083.8568 m")
        
        s = bvm_dist(E, [-30.0*conv, 45.0*conv], [30.0*conv, 45.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 6652788.1287, dp, "Expect d = 6652788.1287 m")
        
        s = bvm_dist(E, [-60.0*conv, 60.0*conv], [5.0*conv, 60.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 7213412.4477, dp, "Expect d = 7213412.4477 m")
        
        s = bvm_dist(E, [-75.0*conv, 85.0*conv], [1.0*conv, 85.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 8442938.5899, dp, "Expect d = 8442938.5899 m")
        
        s = bvm_dist(E, [-80.0*conv, 89.5*conv], [0.0, 89.5*conv], is_radians=True, is_jacobi=True, n=16000)      
    ...
    def test_boundary_value_method_table_4(self):
        ...
        s = bvm_dist(E, [-1.0*conv, 0.0], [0.0, 179.5*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 19884417.8083, dp, "Expect d = 19884417.8083 m")
        
        s = bvm_dist(E, [5.0*conv, 0.0], [-80.0*conv, 170.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 11652530.7514, dp, "Expect d = 11652530.7514 m")
        
        s = bvm_dist(E, [30.0*conv, 0.0], [-75.0*conv, 120.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 14057886.8752, dp, "Expect d = 14057886.8752 m")
        
        s = bvm_dist(E, [60.0*conv, 0.0], [-60.0*conv, 40.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 13767414.8267, dp, "Expect d = 13767414.8267 m")
        
        s = bvm_dist(E, [75.0*conv, 0.0], [-30.0*conv, 0.5*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 11661713.4496, dp, "Expect d = 11661713.4496 m")
        
        s = bvm_dist(E, [80.0*conv, 0.0], [-5.0*conv, 120.0*conv], is_radians=True, is_jacobi=True, n=16000)      
        self.assertAlmostEqual(s[0], 11105138.2902, dp, "Expect d = 11105138.2902 m")
        
        s = bvm_dist(E, [0.0, 0.0], [60.0*conv, 0.0], is_radians=True, is_jacobi=True, n=16000)      
