...
def gen_pol_grid(no_theta=91, no_phi=180, is_connect_8=True, shape=EllipsoidShape(), is_generic=False):
    ...
    # Calculate and store polar grid information.
    grid.no_theta = no_theta
    grid.no_phi = no_phi
    grid.no_vertices = (no_theta - 2)*no_phi + 2
    grid.delta_theta = math.pi / (no_theta - 1)
    grid.delta_phi = 2.0*math.pi / no_phi
    
    # Compute lists of theta and phi values
    grid.theta_list = [0.0] * no_theta
    grid.phi_list = [0.0] * (no_phi + 1)
    for i in range(no_theta):
        grid.theta_list[i] = i * grid.delta_theta
    for i in range(no_phi+1):
        grid.phi_list[i] = i * grid.delta_phi
    ...
    for i in range(grid.no_vertices):
        ...
        carts = np.array( sphere.polar2cart(grid.theta_list[th_index], grid.phi_list[ph_index]) )
    ...
    for i in range(grid.no_vertices):    
        find_neighbour_indices(grid.vertex, i, polar_index[i][0], polar_index[i][1], 
...
def pre_pol_grid(grid, shape):
    # Scale to ellipsoid
    for i in range(grid.no_vertices):
        grid.vertex[i].carts[0] *= shape.a_axis
        grid.vertex[i].carts[1] *= shape.b_axis
        grid.vertex[i].carts[2] *= shape.c_axis
...
def find_neighbour_indices(vertex, i, th, ph, no_theta, no_phi, no_vertices, is_connect_8=True):
            ...
            if i < k0:
                ...
                if th == no_theta-2:
                    ...
                    if ph == 0:
                        ...
                        j_dnlt = get_vertex_index(th+1, ph-1, no_vertices, no_theta, no_phi)
                #vertex[i].neighbour.append(vertex_neighbour)
